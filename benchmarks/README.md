...
answers *none of them*: at a common measured false-alarm rate the three are
indistinguishable, and the ranking they appear to have is an artefact of three different
shipped thresholds. The second one, "[what actually moves the detection
...
density the three shipped CFAR detectors are indistinguishable, and the ranking they
appear to have at their shipped settings is an artefact of three different thresholds.
That answers "which detector should I use" with "it does not matter", and immediately
raises the question this second study measures:
...
* `axes_results.json` -- all of the above plus the settings, the ground-truth hash and
  the full analysis. Depends only on the seeds.
